// En supabase/functions/leave-company/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    if (company.owner_id === user.id) {
      return new Response(JSON.stringify({ error: 'El propietario no puede salir de la empresa, solo eliminarla.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 403,
      })
    }
    ...
    return new Response(JSON.stringify({ message: 'Has salido de la empresa correctamente.' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
  } catch (error) {
