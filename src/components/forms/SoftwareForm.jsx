...
function SoftwareForm({ itemToEdit, onSuccess }) {
  ...
  useEffect(() => {
    if (itemToEdit) {
      const formattedItem = {
        ...
        fecha_vencimiento: itemToEdit.fecha_vencimiento?.split("T")[0] || "",
        proveedor_id:
  ...
  const uploadFile = async (file) => {
    ...
    const fileName = `${activeCompany.id}/software/${Date.now()}-${file.name}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Eliminamos el objeto anidado para evitar errores al actualizar
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <h3 className="text-xl font-semibold mb-4">
        {itemToEdit ? "Editar Software" : "Añadir Software"}
      </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="text"
            name="tipo"
            placeholder="Ej: Licencia Anual, Perpetua"
            value={formData.tipo}
            onChange={handleChange}
            className="input-style"
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <label>Icono o Imagen del Software</label>
          <input
            type="file"
            name="imagen"
