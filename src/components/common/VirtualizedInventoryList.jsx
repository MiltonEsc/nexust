...
const InventoryItem = memo(({ index, style, data }) => {
  const { items, onView, onEdit, onDelete, onSelect, selectedItems } = data;
...
const VirtualizedInventoryList = memo(({
  items = [],
  height = 400,
  itemHeight = 80,
  onView,
  ...
  const listData = useMemo(() => ({
    ...
    onSelect,
    selectedItems,
  }), [items, onView, onEdit, onDelete, onSelect, selectedItems]);
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden">
      <FixedSizeList
        ...
        itemSize={itemHeight}
        itemData={listData}
        overscanCount={5} // Renderizar 5 items extra para scroll suave
      >
        {renderItem}
      </FixedSizeList>
    </div>
